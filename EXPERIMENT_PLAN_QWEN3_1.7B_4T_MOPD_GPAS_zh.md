...
4. **梯度尺度分配**：$q_i\propto\lambda_i s_i$ 能否降低 one-task estimator 的 AdamW-scaled MSE？
5. **成本感知分配**：$q_i\propto\lambda_i s_i/\sqrt{c_i}$ 能否降低“scaled second moment $\times$ GPU time”？
...
| GPAS 降低 estimator MSE | 受控采样、E2 | 理论/Monte Carlo MSE、held-out $M(q)$ | teacher loss vs valid tokens |
| AdamW score 比 raw score 更贴近实际更新 | 受控采样、E3 | 受控 MSE、task-pair ranking、Spearman correlation | GPAS vs Raw-GPAS 附录消融 |
...
3. 用裁剪后的梯度记录 raw / AdamW-scaled score；
...
| P1 | Raw GPAS | $q_i\propto\lambda_i\widehat r_i$ | $\lambda_i/q_i$ | `Raw-GPAS-s42` | 检验 AdamW scaling |
...
P0 是主结论所需的最小端到端矩阵。P1 在 E1--E3 机制检查通过且 P0 轨迹出现对应解释需求时按需选择，不默认全部运行；不用追加方法挤压 P0 的完整评测预算。所有配置均只运行 seed 42 一次。
...
- 第二种 estimator 的显式 task coefficient应为 $1/4$；
...
4. AdamW-scaled gradient coherence：

$$
C_i=\frac{\|\operatorname{mean}(P G_i)\|_2^2}{\operatorname{mean}\|P G_i\|_2^2};
$$

5. corrected mean 对 full-mixture mean 的 relative $\ell_2$ error；
6. corrected mean 对 full-mixture mean 的 cosine；
7. centered AdamW-scaled MSE：

$$
M(q)=\mathbb E\|P\widehat g-Ph\|_2^2;
$$

8. compute criterion：

$$
\left(\sum_iq_ic_i\right)
\left(\sum_i\frac{\lambda_i^2s_i^2}{q_i}\right).
$$

每个 proposal 同时报告相对于 held-out oracle 的 compute-criterion ratio。

### 预测

- 同源 teacher 应保持非平凡 top-16 overlap，log-ratio tail 不应随训练恶化；
- reverse KL 下降的任务通常应伴随 $s_i$ 下降；
- 高 gradient coherence 应与 E3 中正向的 one-step teacher-loss change 一致；
- Corrected GPAS / Cost-GPAS 的 empirical mean 接近 full-mixture mean；
- Uncorrected proposal 的 mean 朝长期高采样任务偏移；
- GPAS 的 held-out $M(q)$ 低于 Uniform 和 Raw GPAS；
- Cost-GPAS 的 compute criterion 低于 Uniform、GPAS 和 Cost-only。

### 机制验收口径

- **Correction**：GPAS 和 Cost-GPAS 的 corrected mean 在至少 4/5 个 checkpoint 上同时满足 cosine $\geq0.99$ 且 relative $\ell_2$ error $\leq0.10$。
- **Estimator**：GPAS 的 held-out $M(q)$ 低于 Uniform；Cost-GPAS 的 held-out compute criterion 低于 Uniform，且对比 GPAS 的方向单独报告。
...
## 7. E3：AdamW score 和 online tracking
...
- one-step loss change 与 $s_i$ 的关系；
- one-step loss change 与 gradient coherence $C_i$ 的关系。

大的 $s_i$ 同时伴随低 coherence 或负 loss change 时，记录为“update scale 未转化为一致局部进展”，不将其直接解释为可学习的 teacher headroom。

报告每 checkpoint 的 task-pair ranking accuracy 和 Spearman correlation。SGD 使用同一批 cached gradient 作为 raw-score 对照。

### Online tracking

`Uniform-s42` 主 run 每 25 updates 记录：

- current held-out score；
- online EMA score；
...
2. **完成受控实验复核**：确认 correction、floor 和 cost criterion 的实现与公式一致。
