...
Table~\ref{tab:checkpoint_selection} summarizes the resulting checkpoint choices and test-score differences.

\input{experiments/completed_results_20260917/selection_table}

Table~\ref{tab:qwen_all_checkpoints} gives all 37 trained-checkpoint development evaluations, including the ten endpoints already reported in Table~\ref{tab:completed_endpoints}. These intermediate measurements evaluate checkpoints along the same ten training trajectories.
\input{experiments/visual_mvp_20260918/qwen_all_checkpoints_table}

The retention-constrained and maximum-mean rules select the same checkpoint in every trajectory. On the additional test sets, the update-250 PG-DT/Adam checkpoint scores 0.366 percentage points below its update-500 endpoint in four-domain mean. The endpoint therefore has the higher separate-test mean in this comparison. All selected models satisfy the retention and useful-gain conditions at the level of test point estimates; joint statistical uncertainty across domains remains unquantified. PG-DR/Adam has two intermediate development checkpoints outside the retention tolerance: GPQA falls from 32.20\% initially to 28.03\% at update 50 and 30.18\% at update 250, before reaching 35.97\% at update 500. The learning curve captures these temporary regressions and their subsequent recovery.
