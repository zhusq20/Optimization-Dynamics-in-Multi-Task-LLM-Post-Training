...
Two properties of the training system strongly affect how parameter changes appear: the precision used to store weights and the optimizer state shared across updates. We examine both when interpreting concentrated checkpoint changes \citep{yu2026denseupdates} and alignment between teacher-specific steps.
...
The precision effect also extends to the concentration of update magnitude. About one third of FP32 coordinates carry 90\% of squared cumulative change, compared with about 4\% of BF16 coordinates. We compute these fractions from unique model coordinates and measure nonzero changes relative to initialization; Appendix~\ref{app:records} gives the definitions and threshold analysis.

Loss averaging provides a further comparison between parameter changes and performance. Under top-64 supervision, the averaging rules produce different fractions of changed BF16 weights while reaching similar final mean scores. The changed fraction describes the stored displacement, and its relationship to task performance depends on the training setting.

\input{sections/fig_nine_sparsity}

\subsection{Shared Adam moments align teacher-specific updates}

Teacher-specific steps are strongly aligned when they start from the same trained student and saved Adam state. On responses from each teacher's own domain, mean update cosines exceed 0.83 under both sampled-token and top-64 supervision. Resetting the first moment while retaining the second, or replacing Adam with norm-matched SGD, reduces the mean cosine to near zero (Figure~\ref{fig:subnetwork_overview}b). The accumulated first moment accounts for much of the alignment.
...
\end{equation}
Both steps start from identical parameters and state, so $\delta U_d$ measures the current gradient's contribution conditional on the saved moments.
