...
SmolLM3-3B diagnostics show that top-64 retains most student probability on average but misses more on some inputs. Across nine batches, it approximates the full-vocabulary gradient more closely than top-4 or top-8, with relative error ranging from 2.16 to 24.54\%. Section~\ref{sec:recipe} connects these measurements to checkpoint selection: a per-domain retention constraint leaves all ten development-selected checkpoints unchanged. Together, the results motivate per-task learning curves alongside gradient and parameter statistics.
