...
TITLES=[
 ...
 ['Input control for teacher gradients','Raw gradient vs BF16 alignment','Zero-gradient control'],
 ['Teacher-pair JS in common order','Task-input update-set distance','Shared-input update-set distance'],
 ['BF16 changed fraction within states','Gradient and update norms relative to PG','Angle to full-vocabulary update'],
...
def main():
    ...
    charts={f['figure']:f for f in manifest['figures']}
    for line in DOC.read_text().splitlines():
        ...
        rows.append({'panel_id':panel,'row':r+1,'column':col,'core_24':r<8,'status':status,
                     'title':TITLES[r][c],'reviewer_question':charts.get(panel,{}).get('question'),
                     'suggested_placement':charts.get(panel,{}).get('suggested_placement'),'claim':CLAIMS[r],'plot_and_required_data':cells[1],
