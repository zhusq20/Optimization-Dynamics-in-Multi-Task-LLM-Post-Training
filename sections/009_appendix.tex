...
\midrule
Student & Qwen3-1.7B \citep{qwen2025qwen3} in non-thinking mode; exact frozen
revision in Table~\ref{tab:v4_frozen_assets}; \missing{initial scores} \\
Teachers & Math and instruction following use task-specific Qwen3-1.7B RL
checkpoints at step 299; code and science use one frozen Qwen3-4B weight set
through separate task endpoints. All run in non-thinking mode and have
byte-identical tokenizer files. Exact identifiers are in
Table~\ref{tab:v4_frozen_assets}; \missing{RL recipe and teacher scores} \\
...
serves rollouts and holds the four resident teachers; \missing{GPU models,
inference engine, software revisions, and GPU hours per run} \\
\bottomrule
\end{tabular}
\end{table}

\input{sections/table_v4_frozen_assets}
...
$m^{\mathcal A}$ (LossGap uses the checkpoint's held-out losses); half
$\mathcal B$ gives $\widehat e^{\mathcal B}_i$, and the
...
tokens. $\tau_i$ is the scoring and backward time of task $i$'s micro-batches
divided by $m_i$; $C$ is the remaining step time, including rollout, weight
synchronization to the inference GPU, and the optimizer update. GPU hours are
wall time times the two reserved GPUs, including all waiting. Checkpoints
store the allocation, noise estimates, timing averages, and prompt positions,
and a resume test reproduces the next allocation and parameter update. A
layout that scores tasks concurrently with training has a step time closer to
$C+\max_i m_i\tau_i$ than to the serial sum; the fixed-objective scaling and
GPAS are unchanged, and Cost-GPAS should use the step-time model fitted for
that layout.
