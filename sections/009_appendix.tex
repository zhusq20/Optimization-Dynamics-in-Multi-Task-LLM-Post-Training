...
Figure~\ref{fig:all_token_shares} supports the loss-weighting comparison in Section~\ref{sec:normalization}. Each branch allocates 25\% of prompts to every domain. Under GT, the plotted token shares also give the domain loss weights; DT and DR keep fixed domain weights. Records at the same rollout clock are merged across metric writes. The first 50 unsmoothed batches expose the early imbalance; capability trajectories continue through update 500.
