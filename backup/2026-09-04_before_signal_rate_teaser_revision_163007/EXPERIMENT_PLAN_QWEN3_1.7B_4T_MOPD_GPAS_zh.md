...
当一个学生能分别从各教师获得能力时，联合 MOPD 的均值更新能否同时改善各域目标？有限样本是否会使本来有用的更新损害某个域？增加样本或改变分配能否可靠地减少这种失败，并改善实际能力或计算效率？

实验先确认单任务可迁移性，再用同一 checkpoint 的干预区分均值方向不利与估计不准确，之后检验预先设定的预测，最后报告能力和计算成本。单任务可学不证明各域参照分数能由一个模型同时达到；局部冲突也不证明联合目标不可达。
...
仅用第一训练 seed 的第 50 步诊断确定规则、回归阈值、置信分析和指标，记录后再查看第 250、450 步及其余两个训练 seeds 的诊断结果。预测目标包括：哪些域更易回归、GPAS 或精确估计能减少多少回归、哪些设置增加样本收益很小。比较方向余量/方差与单独 trace noise 的预测，报告错误和无效干预。验证集不用于重新选择阈值。
...
正常训练日志保留每域 loss/velocity、preconditioned 与原始 trace noise 及 EMA、m_i、prompt 曝光/唯一数、有效 token、回复长度/截断率、各阶段耗时和峰值显存。共同 checkpoint 追加 a_i、方向方差、估计误差和实际干预结果。Cost-aware 结果报告拟合和实测时间及统计开销；TA-OPD 配对仍作为探索性 loss 稳健性比较。

### 稿件结果顺序

1. 单任务可迁移性与联合能力：匹配域曝光和总计算量，报告不确定性。
2. 共同 checkpoint 干预：均值相互作用、采样失败、精确估计和预算扫描。
...
所有占位段落和图表仅写待测量的内容，不填入改善方向、虚构数据或成功结论。
