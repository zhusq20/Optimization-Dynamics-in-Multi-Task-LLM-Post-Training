% HISTORICAL: GPAS-centered theory, not included by the active manuscript as of 2026-09-05.
% ============================================================================
\section{When Can a Shared Update Make Joint Progress?}
\label{sec:theory}
% ============================================================================

\subsection{How teacher information produces a task gradient}
\label{sec:teacher_signal}

An ideal reference distinguishes the teacher's target from the implemented
update. If a sequence-level teacher is exactly
$T_i^*(y\mid x)\propto\pi_0(y\mid x)\exp(R_i(x,y)/\beta_i)$,
minimizing full-sequence reverse KL is equivalent to maximizing expected
reward with KL regularization of strength $\beta_i$, up to a positive
scale and an additive constant. Appendix~\ref{app:ideal_teacher} derives
the identity and its temperature-dependent mixture weights. It requires
an exact teacher for the stated reward and reference. It does not make
our fixed-prefix, response-mean, top-$k$ update a full-sequence RL gradient.

For the implemented loss, let $\nu_i^t$ be the prefix distribution induced
by sampling $(x,y)\sim\mathcal R_i^t$ and then a position uniformly within
$y$. At $\theta_t$, define
\begin{equation}
    s_\theta(c,a)=\nabla_\theta\log p_\theta(a\mid c),\qquad
    z_i(c,a)=\mathbf1_{\{a\in S_i(c)\}}
                   \log\frac{q_i(a\mid c)}{p_\theta(a\mid c)}.
    \label{eq:masked_teacher_signal}
\end{equation}
Differentiating Equation~\ref{eq:dense_topk_loss} and using the
full-vocabulary score identity gives
\begin{equation}
\begin{split}
    -\mu_i
      &=\mathbb E_{\nu_i^t,p_\theta}[z_i s_\theta]
       =\mathbb E_{\nu_i^t,p_\theta}
          [(z_i-\mathbb E[z_i\mid c])s_\theta],\\
    \|\mu_i\|_2
      &\leq\sqrt{\mathbb E_{\nu_i^t}\operatorname{Var}_{p_\theta}
                      (z_i\mid c)}
             \sqrt{\mathbb E_{\nu_i^t,p_\theta}\|s_\theta\|_2^2}.
\end{split}
\label{eq:teacher_signal_projection}
\end{equation}
Teacher differences produce an update through their correlation with the
student's score on visited prefixes. A large signal variance does not
imply a large or useful gradient, and this variance is not micro-batch
gradient covariance. Centering applies to the complete masked signal $z_i$:
subtracting a baseline only inside $S_i(c)$ generally changes the update,
since $\sum_{a\in S_i(c)}p_\theta s_\theta=
\nabla_\theta p_\theta(S_i(c)\mid c)$ need not vanish.
Appendix~\ref{app:masked_signal} gives the proof and teacher-error
qualification. These relations identify how prefix coverage, teacher
probabilities, and retained support determine the mean gradient that
...
\subsection{Common descent can still be slow}
\label{sec:local_rates}

The signs of $a_i$ do not specify the rate of progress. Consider exact
gradients, $D=I$, equal weights, and
\begin{equation}
    L_1(x,y)=\tfrac12x^2,\qquad
    L_2(x,y)=\tfrac{\gamma}{2}y^2,\qquad 0<\gamma<1.
    \label{eq:zero_conflict_losses}
\end{equation}
The task gradients are orthogonal everywhere. From $x(0)=y(0)=1$,
gradient flow for $(L_1+L_2)/2$ gives
\begin{equation}
    \frac{L_1(t)}{L_1(0)}=e^{-t},\qquad
    \frac{L_2(t)}{L_2(0)}=e^{-\gamma t}.
    \label{eq:zero_conflict_rates}
\end{equation}
Both tasks improve at every finite time, with no sampling uncertainty,
yet their decay-rate ratio is $1/\gamma$.
Figure~\ref{fig:joint_progress_teaser}(a) uses $\gamma=0.1$.
More generally, near a common matching solution of fixed-prefix losses,
the local quadratic model has
$\dot e=-D H(w)e$, where $H(w)=\sum_iw_iH_i$.
For the corrected top-$k$ loss, each matching-point $H_i$ is the
score second moment on the retained support, not generally the full
Fisher matrix. Appendix~\ref{app:local_rates} states the assumptions.

This example establishes a possible cause of slow progress, not a
convergence model for the full MOPD run. Small raw gradient inner products
also do not establish improved angles: shrinking opposed gradients have
inner products tending to zero with cosine $-1$, while nonzero orthogonal
gradients already have zero inner product.
Changing the task weights can change deterministic rates; changing counts
$m_i$ at fixed weights preserves the local mean field. The separable
minimum-time weight calculation in Appendix~\ref{app:static_allocation}
therefore does not justify GPAS's sample allocation.
