...
OUT=HERE/'figure_audit_20260908'
FIG=HERE.parent/'figures/evidence_audit_20260908'
TITLES=[
 ...
 ['Teacher support overlap','Signed step alignment','Zero-gradient control'],
...
PACKAGES=['W4','W4','W2,W3,W7','W2,W3,W7','W5','W5','W5','W3,W6,W7','W7,W8']
DATA_KEYS=['rollout_domain_metrics + normalization_probes','normalization_probes','none: TopK capability missing',
           'bf16_scans + cumulative_measurements','local_probes + teacher_pairs','local_probes + teacher_pairs',
           'local_probes','evaluations + runs; online BF16 steps missing','none: target robustness experiments missing']

def main():
    rows=[]
    for line in DOC.read_text().splitlines():
        match=re.match(r'\| \*\*(F([1-9])([abc])) \[([EPN])\]\*\*',line)
    ...
    assert Counter(r['status'] for r in rows[:24])=={'E':12,'P':8,'N':4}
    ...
    palette={'E':('#e6f2ee','#20735a'),'P':('#fff3df','#9b6218'),'N':('#eef0f4','#586374')}
    label={'E':'EXPLORATORY DATA AVAILABLE','P':'PARTIAL DATA','N':'TARGET DATA STILL MISSING'}
