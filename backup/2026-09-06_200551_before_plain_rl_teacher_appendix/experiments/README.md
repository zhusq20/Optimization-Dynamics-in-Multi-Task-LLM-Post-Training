...
Teacher-conditioned gradients and proposed optimizer steps start from identical copies of an actual MOPD checkpoint. Independent single-teacher endpoint deltas are reference measurements, not historical attribution inside the joint run. Mathematics, code, instruction following, and science each use their own frozen Qwen3-1.7B teacher trained with reinforcement learning (RL), giving four teacher weight sets. The student is initialized from Qwen3-1.7B-Base. The theoretical appendix distinguishes each teacher's RL KL reference from this student initialization; their equality and the ideal teacher optimum are not assumed for the experimental interpretation.
