...
fraction of squared change carried by the largest coordinates. We use FP32
master-weight differences when available, with exported BF16 checkpoint
differences reported separately. Local comparisons use proposed steps from
...
\paragraph{Preliminary observation: distribution drift and update sparsity.}
In the exploratory seed-$42$ runs, teacher top-$64$ training exhibits
student probability drift outside the supervised token set. Comparing
rollout batches $1$--$50$ with $200$--$249$, retained student mass falls
from $86.2\%$ to $37.0\%$ for the single teacher and from $81.4\%$ to
$51.7\%$ for joint training under domain-response (DR) averaging.
With domain-token and global-token averaging, joint retained mass instead
rises from $88.2\%$ to $93.7\%$ and from $88.6\%$ to $94.9\%$,
respectively. These logged means use each run's on-policy prefixes and
configured averaging. The stronger drift under DR is consistent with
response averaging amplifying truncation effects. Actual FP32 updates
show no exceptional concentration at the measured checkpoints: at update
$250$, the largest $1\%$ of coordinates carries $8.17\%$ of update energy
for joint top-$64$ DR, versus $9.03\%$, $9.03\%$, and $8.32\%$ for PG,
domain-token, and global-token. The corresponding cumulative-change
shares are $8.59\%$, $11.38\%$, $8.24\%$, and $8.24\%$.
The association remains to be tested with controlled interventions.
% Audited rollout windows, FP32/BF16 measurements, source hashes, and
% W&B/local checks: experiments/topk_drift_sparsity_20260906.json
