...
losses in Section~\ref{sec:setting}. Let $\mathcal B_d$ contain the
...
\midrule
All tokens together
  & $\displaystyle\frac{\sum_i |y_i|\ell_i}{\sum_i |y_i|}$ \\
Tokens within each domain
  & $\displaystyle\sum_d\lambda_d
    \frac{\sum_{i\in\mathcal B_d}|y_i|\ell_i}
         {\sum_{i\in\mathcal B_d}|y_i|}$ \\
Responses within each domain
  & $\displaystyle\sum_d\lambda_d
    \frac{1}{|\mathcal B_d|}\sum_{i\in\mathcal B_d}\ell_i$ \\
\bottomrule
\end{tabular}
...
All three studies use a Qwen3-1.7B student on mathematics, code,
instruction following, and science. Math and instruction following have
domain-specific Qwen3-1.7B RL teachers; code and science currently share
one Qwen3-4B teacher. The reference run uses equal domain prompt quotas
...
The experiments below are planned; measured results are not yet available.
